...
export const ChangePasswordCompletedForm = memo<ChangePasswordCompletedFormProps>(({ className }) => {
    ...
    return {
      ...
      validate: (values) => {
        ...
        if (isNotDefinedString(values.newPassword)) {
          errors.newPassword = t(`errorsForm.is_required`);
        }
        if (isNotDefinedString(values.repeatPassword)) {
          errors.repeatPassword = t(`errorsForm.is_required`);
        }
        if (values.repeatPassword !== values.newPassword) {
          errors.newPassword = t(`errorsForm.not_same_password`);
          errors.repeatPassword = t(`errorsForm.not_same_password`);
        }
        return errors;
      },
    };
  ...
  return (
    <div className={cn(s.root, className)}>
      <Title className={s.title}>{t(`changePassword.title`)}</Title>
